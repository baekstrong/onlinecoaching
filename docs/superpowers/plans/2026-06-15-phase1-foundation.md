...
> ⚠️ **보안 강화 반영됨(실행 후 갱신):** 아래 SQL은 초기 버전이며, 실행 과정에서 보안 리뷰로 RLS가 강화되었다(role 자가변경 차단, 첨부 게이트, 요청 생성 필드 고정, 템플릿 코치-소유 한정, 최소권한 grant, `is_coach()`에 `security definer`). **실제 기준은 커밋된 `supabase/migrations/0001_init.sql` 파일**이다. 새로 실행한다면 그 파일을 사용할 것.
